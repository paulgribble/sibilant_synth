...
- **Sibilant = morph, not cross-fade.** Whole time-varying spectra (5 slices,
  1/48-oct log grid) are warped piecewise-linearly in log f so the two
  landmarks coincide at `(1−a)·u_ʃ + a·u_s`, then levels are interpolated.
  A plain dB mix gives bimodal spectra at a ≈ 0.5.
...
- Gotchas hit: `interp1` on a single spectrum returns a row that transposes
  ...
  not valid MATLAB.
- `test_output/` is regenerated by the test/validate scripts; safe to delete.
  It is gitignored (along with `*.asv`, `*.autosave`, `slprj/`, `.DS_Store`),
  so a fresh clone has none of it. `sibilant_model.mat` IS tracked.
