...
%   BreakEvery  offer a self-paced break every this many trials (default 0 = never)
...
%       participant, trial, block (repetition number), talker, vowel, a,
...
stim = PreparePerceptionStimuli(opts.StimDir, talkers, vowels, A, 'Regenerate', opts.Regenerate);
nStim = height(stim);
wav = cell(nStim, 1);
wavFs = zeros(nStim, 1);
if ~simulate
...
order = zeros(nStim, opts.Reps);
for r = 1:opts.Reps, order(:, r) = randperm(rs, nStim); end
block = repmat(1:opts.Reps, nStim, 1);
order = order(:);  block = block(:);
nTrials = numel(order);
...
info = struct('participant', participant, 'started', string(datetime('now')), 'completed', false, ...
              'nTrialsPlanned', nTrials, 'nTrialsDone', 0, 'A', A, 'reps', opts.Reps, ...
...
fprintf(fid, "participant\ttrial\tblock\ttalker\tvowel\ta\tfilename\tresponse\tresp_s\tword\tinput\trt_s\tsh_side\ttime\n");
...
if simulate
    for it = 1:nTrials
        p = opts.Simulate(3) + (1 - 2 * opts.Simulate(3)) / (1 + exp(-(stim.a(order(it)) - opts.Simulate(1)) / opts.Simulate(2)));
        writeTrial(it, order(it), rand(rs) < p, "sim", NaN);
    end
else
    runGui();
end
...
if nDone > 0
    [g, lev] = findgroups(T.a);
    fprintf("  a        %s\n  p(""s"")   %s\n", sprintf("%6.3f", lev), sprintf("%6.2f", splitapply(@mean, T.resp_s, g)));
end
...
% =========================================================================
    ...
    function writeTrial(t, k, saidS, how, rt)
        resp = ["sh" "s"];  resp = resp(saidS + 1);
        if stim.vowel(k) == "i", words = ["she" "see"]; else, words = ["shoe" "sue"]; end
        fprintf(fid, "%s\t%d\t%d\t%s\t%s\t%.3f\t%s\t%s\t%d\t%s\t%s\t%.3f\t%s\t%s\n", participant, t, block(t), ...
