...
| `sibilant_model.mat` | the learned model, 72 talkers, ~22 MB (rebuilt 2026-09-18 to rename the per-talker label `gender` → `sex`; tokens unchanged) |
...
per-talker sex formant presets (nothing else changed):
...
- Gender only affects the formant-tracking presets used in validation, not
  the synthesis. The roster's `sex` column is the experimenter's by-ear
  rating of each voice (2026-09-18), not self-report; it was a `male`
  placeholder before that, and the experiment repo's `all_extracted.tsv`
  was extracted with the placeholder. Future participants will be asked
  their sex and gender; `sex` remains the acoustic-preset variable.
